...
class EnergyEmployeeAnalytics extends Component {
    constructor(props) {
        super(props)
    ...
    async Loader() {
        ...
        try {
            ...
            for (let i = 0; i < res.data.stations.length; i++) {
                temp.push(res.data.stations[i].name);
            }
            this.setState({stationNames: temp});
            temp = ['all'];
            for (let i = 0; i < res.data.stations.length; i++) {
                temp.push(res.data.stations[i].stationId);
            }
            ...
            try {
                ...
                for (let i = 0; i < res.data.length; i++) {
                    for (let j = 0; j < res.data[i].sessions.length; j++) {
                        if (res.data[i].sessions[j].start_date.slice(5, 7) === month) {
                            let day = res.data[i].sessions[j].start_date.slice(8, 10);
                            if (day >= '01' && day < '08') {
                                totalSessionstemp[0]++;
                                kWtemp[0] += res.data[i].sessions[j].energy_delivered;
                            }
                            else if (day >= '08' && day < '15') {
                                totalSessionstemp[1]++;
                                kWtemp[1] += res.data[i].sessions[j].energy_delivered;
                            }
                            else if (day >= '15' && day < '22') {
                                totalSessionstemp[2]++;
                                kWtemp[2] += res.data[i].sessions[j].energy_delivered;
                            }
                            else if (day >= '22') {
                                totalSessionstemp[3]++;
                                kWtemp[3] += res.data[i].sessions[j].energy_delivered;
                            }
                        }
                    }
                }
                ...
                for (let i = 0; i < res.data.length; i++) {
                    for (let j = 0; j < res.data[i].sessions.length; j++) {
                        for (let k = 0; k < this.state.months.length; k++) {
                            if (res.data[i].sessions[j].start_date.slice(5, 7) === this.state.months[k]) {
                                let day = res.data[i].sessions[j].start_date.slice(8, 10);
                                if (day >= '01' && day < '08') {
                                    totalSessionstemp[4*k]++;
                                    kWtemp[4*k] += res.data[i].sessions[j].energy_delivered;
                                }
                                else if (day >= '08' && day < '15') {
                                    totalSessionstemp[4*k+1]++;
                                    kWtemp[4*k+1] += res.data[i].sessions[j].energy_delivered;
                                }
                                else if (day >= '15' && day < '22') {
                                    totalSessionstemp[4*k+2]++;
                                    kWtemp[4*k+2] += res.data[i].sessions[j].energy_delivered;
                                }
                                else if (day >= '22') {
                                    totalSessionstemp[4*k+3]++;
                                    kWtemp[4*k+3] += res.data[i].sessions[j].energy_delivered;
                                }
                            }
                        }
                    }
                ...
                for (let i = 0; i < res.data.length; i++) {
                    for (let j = 0; j < res.data[i].sessions.length; j++) {
                        for (let k = 0; k < this.state.months.length; k++) {
                            if (res.data[i].sessions[j].start_date.slice(5, 7) === this.state.months[k]) {
                                totalSessionstemp[k]++;
                                kWtemp[k] += res.data[i].sessions[j].energy_delivered;
                            }
                        }
                    }
                }
